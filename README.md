...
6. **Bracket**; Head to head matchups (highest score vs lowest score, second highest vs second lowest, etc), players vote for their favorite to win each round. Possible addition of timers which can be turned off in the settings. 7.**Complete**: The winner is crowned, the result is saved to the game history, the players are returned to the lobby
...
- [ ] Fix header text display: Figure out why there's strange styling on the text for different browsers
...
- [ ] Players rank their top N favorites (X = number of items, either N = X _ 0.5 or N = X _ 0.25)
